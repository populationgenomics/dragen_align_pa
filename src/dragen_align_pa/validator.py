...
from dragen_align_pa.constants.constants import DESIGN_TO_BEDS, DESIGN_TO_CANONICAL
...
def _resolve_sg_canonical_design(sg: SequencingGroup) -> str:
    ...
    if sequencing_library not in DESIGN_TO_CANONICAL:
        raise RuntimeError(
            ...
            f'dragen_align_pa.constants.constants.',
        )
...
def assert_cohort_design_matches_configured_bed(cohort: Cohort) -> None:
    ...
    if valid_beds is None:
        raise RuntimeError(
            f'No DESIGN_TO_BEDS entry for design {cohort_design!r}; update dragen_align_pa.constants.constants.',
        )
