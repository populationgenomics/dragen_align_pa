...
def _setup_paths(
    ...
    Uses DRAGEN-generated CRAMs when
    `[dragen_align_pa.upload_data_to_ica.use_dragen_crams]` is true. Returns a dict with
    keys 'sg_name', 'cram_name', 'gcs_cram_path', 'local_cram_path', and 'ica_folder_path'.

    Raises:
        ValueError: In non-DRAGEN mode, if `sequencing_group.cram.path` is neither a
            '.cram' nor a '.cram.crai' path.
    """
