    ...
    job.command(
        command(
            f"""
                ...
                md5=$(icav2 projectdata list --parent-folder /{bucket}/{ica_analysis_output_folder}/{sg_name}/{sg_name}_${{ar_guid}}_-${{pipeline_id}}/{sg_name}/ --data-type FILE --file-name {sg_name}.{data}.md5sum --match-mode EXACT -o json | jq -r '.items[].id')
