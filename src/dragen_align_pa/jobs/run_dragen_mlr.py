...
def _initalise_mlr_job(sequencing_group: SequencingGroup) -> BashJob:
    mlr_job: BashJob = get_batch().new_bash_job(
        name='MlrWithDragen',
...
def run_mlr(
    ...
    job: BashJob = _initalise_mlr_job(sequencing_group=sequencing_group)
    ...
    job.command(
        f"""
                    ...
                    cram="ica://OurDNA-DRAGEN-378$(icav2 projectdata list --parent-folder /{bucket}/{ica_analysis_output_folder}/{sequencing_group.name}/{sequencing_group.name}_${{ar_guid}}_-${{pipeline_id}}/{sequencing_group.name}/ --data-type FILE --file-name {sequencing_group.name}.cram --match-mode EXACT -o json | jq -r '.items[].details.path)"
                    gvcf="ica://OurDNA-DRAGEN-378$(icav2 projectdata list --parent-folder /{bucket}/{ica_analysis_output_folder}/{sequencing_group.name}/{sequencing_group.name}_${{ar_guid}}_-${{pipeline_id}}/{sequencing_group.name}/ --data-type FILE --file-name {sequencing_group.name}.hard-filtered.gvcf.gz --match-mode EXACT -o json | jq -r '.items[].details.path)"
