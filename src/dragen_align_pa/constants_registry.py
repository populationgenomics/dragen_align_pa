...
def resolve_cnv_normals_panel(panel_name: str) -> tuple[str, list[str]]:
    ...
    if not isinstance(count_ids, list) or not all(isinstance(fid, str) and fid for fid in count_ids):
        raise ValueError(
            f'CNV panel {panel_name!r} {_PON_COUNT_KEY!r} must be a list of non-empty ICA file ID '
            f'strings, got {count_ids!r}. Rebuild it with scripts/build_cnv_panel_of_normals.py.',
        )
