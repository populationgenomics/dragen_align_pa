...
# Only file IDs are stored, never the count-file basenames: those embed CPG sample IDs (blocked
# by the CPG-ID pre-commit hook) and are unneeded here — ICA localises each file by its
# registered name and DRAGEN reads filenames from the `.normals.txt` list at runtime. The
# submitter sends every ID as `additional_files` data inputs and derives the --cnv-normals-list
# basename as `<panel>.normals.txt`. Built by scripts/build_cnv_panel_of_normals.py; a run
# selects a panel via [presets.exome].cnv_normals_panel. Resolve via resolve_cnv_normals_panel.
