...
Config files should be reviewed and merged into the `production-pipelines-configuration` repository prior to running on production data. Registered ICA project families (valid `ica.projects.project_root` values) are the top-level keys of `ICA_PROJECT_SETUP` in [`ica_constants.py`](src/dragen_align_pa/constants/ica_constants.py).
Valid entries for config settings such as `dragen_align_pa.manage_dragen_pipeline.presets.exome.bed_names` can be found in [`ica_constants.py`](src/dragen_align_pa/constants/ica_constants.py).
...
Your TOML configuration file must specify the following key options:
  ...
  * `[workflow]`:
   ...
   * **If `reads_type = "cram"`:**
      ...
      * `[ica.cram_references]`:
          * `reference`: Must be set to one of the defined references in [`ica_constants.py`](src/dragen_align_pa/constants/ica_constants.py). Current valid options are `hg38_masked.fasta` and `hg38_unmasked.fasta` e.g. `reference = 'hg38_masked.fasta'`.
   * **If `reads_type = "fastq"`:**
...
- Run it (see `--help` for all flags):
    ...
    ```
**Usage**
- Register the panel: merge the emitted block into `ICA_PON_FILE_IDS` in `src/dragen_align_pa/constants/ica_constants.py` (reformat to the repo code style — the emitted block is JSON, not Python).
