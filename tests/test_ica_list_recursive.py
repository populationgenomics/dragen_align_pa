"""Unit tests for `ica_utils.list_ica_files_recursive`.

The helper walks an ICA folder tree by alternating FILE and FOLDER list calls
at each level. These tests use a MagicMock api_instance configured to respond
to (parentFolderPath, type) pairs deterministically — no live ICA calls.
"""
...
from dragen_align_pa.ica_utils import list_ica_files_recursive
...
def _make_api(children: dict[tuple[str, str], list[dict]]) -> MagicMock:
    ...
    in these tests — see the dedicated pagination test).
    """
...
def test_recursive_list_returns_relative_paths():
...
def test_recursive_list_empty_base_folder_returns_empty_list():
    api = _make_api({})
    result = list_ica_files_recursive(
        api_instance=api, path_parameters={'projectId': 'p'}, base_ica_folder_path='/empty/',
    )
    assert result == []


def test_recursive_list_normalises_trailing_slash():
...
def test_recursive_list_skips_items_with_missing_name_or_id():
...
def test_recursive_list_propagates_api_exception_mid_walk():
...
def test_recursive_list_handles_pagination_for_files():
